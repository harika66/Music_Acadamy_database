...
docker_setup_db() {
	...
	if [ -f "$MYSQL_DUMP_FILE" ]; then
		mysql_note "Starting sql dump from ${MYSQL_DUMP_FILE}"
		docker_process_sql --database=${MYSQL_DATABASE} <<<"SOURCE $MYSQL_DUMP_FILE;"
	fi
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			mysql_note "Root password is ${MYSQL_ROOT_PASSWORD} DB user is "$MYSQL_USER" and user password is "$MYSQL_PASSWORD""
			docker_init_database_dir "$@"
